...
fn main() {
    ...
    println!(
        "  range corners hit: {saturated} inf, {flushed} zero, {subnormal} subnormal"
    );
    ...
    println!(
        "PASS: from_stream is bit-identical to from_host + device cast_f32_to_f16"
    );
}

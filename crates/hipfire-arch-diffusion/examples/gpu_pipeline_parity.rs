...
fn max_rel(a: &[f32], b: &[f32], tag: &str) -> f32 {
    ...
    let max_err = a.iter().zip(b.iter()).fold(0.0f32, |m, (x, y)| m.max((x - y).abs()));
    let rel = max_err / max_want;
    println!("  {tag} len={} max_err={max_err:.3e} rel={rel:.3e}", a.len());
    rel
}
...
fn psnr(a: &[f32], b: &[f32]) -> f64 {
    ...
    mse /= a.len() as f64;
    let psnr = if mse == 0.0 { f64::INFINITY } else { 20.0 * (1.0 / mse.sqrt()).log10() };
...
fn main() {
    ...
    let cpu = generate_txt2img_prompt(
        &bundle, prompt, width, height, steps, seed, &mut noop,
    )
    ...
    let gpu_out = generate_txt2img_prompt_gpu(
        &mut bundle, &mut gpu, prompt, width, height, steps, seed, &mut noop,
    )
    ...
    if cpu.steps.len() != gpu_out.steps.len() {
        eprintln!("FAIL: step count {} != {}", cpu.steps.len(), gpu_out.steps.len());
        std::process::exit(1);
    }
    ...
    println!("  png_identical={png_identical} ({}, {})", cpu.png.len(), gpu_out.png.len());

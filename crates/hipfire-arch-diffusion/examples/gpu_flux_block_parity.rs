...
use hipfire_arch_diffusion::flux::{
    decode_dtype, double_block, single_block, Tensor, FluxWeights, MlpAct,
};
...
fn cmp(label: &str, gpu_v: &[f32], cpu_v: &[f32]) -> usize {
    ...
    let max_err = gpu_v.iter().zip(cpu_v.iter()).fold(0.0f32, |m, (a, b)| m.max((a - b).abs()));
...
fn main() {
    ...
    let needed: Vec<_> = keys
        .iter()
        .filter(|k| {
            k.name.starts_with("double_blocks.0.")
                || k.name.starts_with("single_blocks.0.")
        })
        .collect();
    ...
    eprintln!("host: {} block-0 tensors decoded (BF16→f32)", host.tensors.len());
    ...
    for k in &needed {
        ...
        upload_flux_key(&mut gpu, &k.name, &t.data, [k.rows, k.cols], &mut gw.tensors)
    ...
    let s_g =
        gpu_single_block(&mut gpu, &cfg, &gw, 0, &g_fused, &g_vec, n_img, grid, MlpAct::GeluTanh)
    ...
    println!("PASS: GPU block parity at real FLUX.1-dev geometry ({})", path);
}

...
impl CondCache {
    ...
    pub fn new() -> Self {
        let enabled = std::env::var("HIPFIRE_IMG_COND_CACHE").map_or(true, |v| v != "0");
        Self {
            entries: Vec::new(),
...
impl FluxPipeBundle {
    ...
    pub fn transformer_host(&self) -> Result<Cow<'_, FluxWeights>, String> {
        ...
        Ok(Cow::Owned(
            s.transformer_plan
                .materialize(s.transformer.as_ref(), &self.transformer_cfg)?,
        ))
    }
                    ...
                    match why {
                        ...
                        None => {
                            let up = match (t5_plan, &self.sources) {
                                (Some(p), Some(s)) => GpuT5Weights::from_stream(gpu, s.text.as_ref(), p),
...
pub fn img_profile_enabled() -> bool {
    std::env::var("HIPFIRE_IMG_PROFILE").is_ok_and(|v| v != "0")
}
...
pub fn text_encoders_on_gpu() -> bool {
    std::env::var("HIPFIRE_T5_GPU").map_or(true, |v| v != "0")
}
...
pub fn load_pipe(pipe_dir: &Path) -> Result<FluxPipeBundle, String> {
    ...
    let config_only = std::env::var("HIPFIRE_VAE_CONFIG_ONLY").is_ok_and(|v| v != "0");
...
pub fn load_pipe_hfq(
    ...
    let (text_hfq, clip_hfq, vae_hfq) = match (flux2, sidecars) {
        ...
        (false, HfqSidecars::Flux2 { .. }) => {
            return Err("flux (HFQ): the trunk is FLUX.1 (arch 40) but the sidecars are a \
                        FLUX.2 Klein set (qwen3 + vae); it needs t5 + clip + vae"
                .into())
        }
        (true, HfqSidecars::Flux1 { .. }) => {
            return Err("flux (HFQ): the trunk is FLUX.2 Klein (arch 45) but the sidecars \
...
fn denoise_and_decode(
    ...
    let family_profile = std::env::var_os("HIPFIRE_PROFILE").is_some();
        ...
        let noise_pred = flux_gpu::gpu_forward_txt_dev(
            ...
            &flux::FluxForwardInput {
                ...
                guidance: if !cfg.is_flux2() && cfg.guidance_embed_dim > 0 {
                    Some(
                        std::env::var("HIPFIRE_FLUX_GUIDANCE")
                            .ok()
    ...
    if std::env::var("HIPFIRE_VAE_CONFIG_ONLY").is_ok_and(|v| v != "0") {
    ...
    let image = if std::env::var("HIPFIRE_VAE_GPU").map_or(true, |v| v != "0") {

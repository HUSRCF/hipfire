...
impl ClipWeights {
    pub fn load(src: &dyn ModelSourceTrait) -> Result<Self, String> {
        let v: serde_json::Value =
            serde_json::from_str(src.metadata_json()).map_err(|e| format!("clip: config.json invalid: {e}"))?;
        ...
        let load = |name: &str, rows: usize, cols: usize| -> Result<Tensor, String> {
            ...
            if n != rows * cols {
                return Err(format!("clip: tensor `{name}` has {n} elems, expected {}", rows * cols));
            }
            Ok(Tensor { data: crate::flux::decode_dtype(&info.dtype, &data)?, rows, cols })
        };
        let num_layers = config.num_hidden_layers;
        let intermediate = config.intermediate_size;
        let mut w = ClipWeights {
            token_embed: load("text_model.embeddings.token_embedding.weight", config.vocab_size, h)?,
            pos_embed: load("text_model.embeddings.position_embedding.weight", config.max_position_embeddings, h)?,
            ln1_w: vec![],
            ln1_b: vec![],
            q_w: vec![],
            q_b: vec![],
        ...
        for i in 0..num_layers {
            ...
            w.q_w.push(load(&format!("{p}self_attn.q_proj.weight"), h, h)?);
            w.q_b.push(load(&format!("{p}self_attn.q_proj.bias"), h, 1)?);
            w.k_w.push(load(&format!("{p}self_attn.k_proj.weight"), h, h)?);
            w.k_b.push(load(&format!("{p}self_attn.k_proj.bias"), h, 1)?);
            w.v_w.push(load(&format!("{p}self_attn.v_proj.weight"), h, h)?);
            w.v_b.push(load(&format!("{p}self_attn.v_proj.bias"), h, 1)?);
            w.out_w.push(load(&format!("{p}self_attn.out_proj.weight"), h, h)?);
            w.out_b.push(load(&format!("{p}self_attn.out_proj.bias"), h, 1)?);
            w.ln2_w.push(load(&format!("{p}layer_norm2.weight"), h, 1)?);
            w.ln2_b.push(load(&format!("{p}layer_norm2.bias"), h, 1)?);
            w.fc1_w.push(load(&format!("{p}mlp.fc1.weight"), intermediate, h)?);
            w.fc1_b.push(load(&format!("{p}mlp.fc1.bias"), intermediate, 1)?);
            w.fc2_w.push(load(&format!("{p}mlp.fc2.weight"), h, intermediate)?);
...
pub fn encode(w: &ClipWeights, input_ids: &[u32], attention_mask: &[u8]) -> (Vec<f32>, Vec<f32>, Vec<Vec<f32>>) {
    ...
    for i in 0..cfg.num_hidden_layers {
        ...
        let fc2 = nn::linear(&gelu, len, cfg.intermediate_size, &w.fc2_w[i], Some(&w.fc2_b[i]));
        for r in 0..len {
            for c in 0..h {

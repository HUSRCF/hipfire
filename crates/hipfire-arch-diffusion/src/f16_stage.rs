...
mod tests {
    ...
    fn rne_matches_reference_for_every_bf16_pattern() {
        ...
        for bits in 0u16..=u16::MAX {
            ...
            if v.is_nan() {
                assert!(f16::from_bits(f32_to_f16_rne(v)).is_nan(), "bf16 {bits:#06x}");
                continue;
            }
    ...
    fn rne_matches_reference_around_every_rounding_boundary() {
        ...
        for bits in 0u16..0x7C00u16 {
            ...
            for v in [mid, f32::from_bits(mid.to_bits() - 1), f32::from_bits(mid.to_bits() + 1)] {
    ...
    fn push_appends_each_dtype_and_reuses_the_buffer() {
        ...
        assert_eq!(stage.capacity_bytes(), cap, "clear must keep the allocation");
    }

...
impl GpuVaeDecoderWeights {
    ...
    pub fn from_host(gpu: &mut Gpu, host: &VaeDecoderWeights) -> Result<Self, String> {
        let gemm = std::env::var("HIPFIRE_VAE_CONV").map_or(true, |v| v != "direct");
...
impl<'a> Run<'a> {
    fn new(gpu: &'a mut Gpu, groups: usize, conv_gemm: bool) -> Self {
        let prof = std::env::var("HIPFIRE_VAE_PROFILE").map_or(false, |v| v != "0");
        let fuse_norm = std::env::var("HIPFIRE_VAE_FUSE_NORM").map_or(true, |v| v != "0");
        let budget_mb = std::env::var("HIPFIRE_VAE_IM2COL_MB")
            .ok()

...
pub fn f16_activations_enabled() -> bool {
    ...
    *V.get_or_init(|| std::env::var("HIPFIRE_FLUX_F16_ACT").map_or(true, |v| v != "0"))
}
...
pub fn mod_gemv_enabled() -> bool {
    static V: OnceLock<bool> = OnceLock::new();
    *V.get_or_init(|| std::env::var("HIPFIRE_FLUX_MOD_GEMV").map_or(true, |v| v != "0"))
}
...
fn step_profile_enabled() -> bool {
    static V: OnceLock<bool> = OnceLock::new();
    *V.get_or_init(|| std::env::var_os("HIPFIRE_PROFILE").is_some())
}
...
///
/// Cached like every other per-process flag in this file (`env::var` +
...
fn flux_attn_route_family(gpu: &Gpu) -> &'static str {
    ...
    *V.get_or_init(|| {
        let forced = std::env::var("HIPFIRE_FLUX_ATTN").ok();
...
fn gemm_lds_enabled() -> bool {
    static V: OnceLock<bool> = OnceLock::new();
    *V.get_or_init(|| std::env::var("HIPFIRE_FLUX_GEMM_LDS").map_or(true, |v| v != "0"))
}
...
fn gemm_wide_enabled() -> bool {
    static V: OnceLock<bool> = OnceLock::new();
    *V.get_or_init(|| std::env::var("HIPFIRE_FLUX_GEMM_WIDE").map_or(true, |v| v != "0"))
}
...
fn weight_pad() -> usize {
    ...
    *V.get_or_init(|| match std::env::var("HIPFIRE_FLUX_WPAD") {
        Ok(v) => {
            let pad: usize = v.parse().unwrap_or_else(|_| {
                panic!("HIPFIRE_FLUX_WPAD must be a non-negative integer of elements, got `{v}`")
            });
            assert!(
                pad % 16 == 0,
...
impl<'a> Gpuf<'a> {
    fn new(
        ...
        let v5 = std::env::var("HIPFIRE_FLUX_ATTN").is_ok_and(|v| v == "v5");

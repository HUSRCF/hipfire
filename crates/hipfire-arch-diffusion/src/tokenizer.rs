...
impl UnigramVocab {
    pub fn from_tokenizer_json(v: &serde_json::Value) -> Result<Self, String> {
        let model = v
            .get("model")
            .ok_or("t5 tokenizer.json: missing model")?;
...
impl Gpt2Bpe {
    ...
    pub fn load(dir: &Path) -> Result<Self, String> {
        let vocab_json: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string(dir.join("vocab.json")).map_err(|e| format!("clip vocab.json: {e}"))?,
        )
        .map_err(|e| format!("clip vocab.json invalid: {e}"))?;
        let merges_raw = std::fs::read_to_string(dir.join("merges.txt")).map_err(|e| format!("merges.txt: {e}"))?;
    ...
    pub fn from_parts(vocab_json: &serde_json::Value, merges_raw: &str) -> Result<Self, String> {
        ...
        let eot_id = *vocab.get("<|endoftext|>").ok_or("clip vocab: missing <|endoftext|>")?;
        let bos_id = *vocab
            .get("<|startoftext|>")
        ...
        Ok(Self {
            ...
            byte_decoder: bytes_to_unicode().into_iter().map(|(k, v)| (v, k)).collect(),
            bos_id,
            eot_id,
            max_len: 77,
        })

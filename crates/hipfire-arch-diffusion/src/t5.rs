...
impl T5Plan {
    pub fn detect(src: &dyn ModelSourceTrait) -> Result<Self, String> {
        ...
        let v: serde_json::Value =
            serde_json::from_str(config_str).map_err(|e| format!("t5: config.json invalid: {e}"))?;
    ...
    pub fn ffn_in(&self, i: usize) -> Vec<String> {
        if self.gated {
            ...
            vec![format!("encoder.block.{i}.layer.1.DenseReluDense.wi.weight")]
        }
    }
    ...
    pub fn materialize(&self, src: &dyn ModelSourceTrait) -> Result<T5Weights, String> {
        ...
        for i in 0..self.config.num_layers {
            out.q.push(self.tensor(src, &format!("encoder.block.{i}.layer.0.SelfAttention.q.weight"), d, d)?);
            out.k.push(self.tensor(src, &format!("encoder.block.{i}.layer.0.SelfAttention.k.weight"), d, d)?);
            out.v.push(self.tensor(src, &format!("encoder.block.{i}.layer.0.SelfAttention.v.weight"), d, d)?);
            out.o.push(self.tensor(src, &format!("encoder.block.{i}.layer.0.SelfAttention.o.weight"), d, d)?);
            out.attn_norm.push(self.tensor(src, &format!("encoder.block.{i}.layer.0.layer_norm.weight"), d, 1)?);
            let ffn_in = self.ffn_in(i);
            out.wi.push(self.tensor(src, &ffn_in[0], d_ff, d)?);
            if self.gated {
                out.wi_gate.push(self.tensor(src, &ffn_in[1], d_ff, d)?);
            }
            out.wo.push(self.tensor(src, &format!("encoder.block.{i}.layer.1.DenseReluDense.wo.weight"), d, d_ff)?);
            out.ffn_norm.push(self.tensor(src, &format!("encoder.block.{i}.layer.1.layer_norm.weight"), d, 1)?);
        }
        Ok(out)
    }
}
...
pub fn relative_position_bucket(relative_position: isize, num_buckets: usize, max_distance: usize) -> usize {
...
pub fn compute_rel_bias(query_len: usize, key_len: usize, rel_bias: &Tensor, max_distance: usize) -> Vec<f32> {
...
pub fn encode(w: &T5Weights, input_ids: &[u32], attention_mask: &[u8]) -> (Vec<f32>, Vec<Vec<f32>>) {
    let cfg = &w.config;
        ...
        for h in 0..heads {
            ...
            for qpos in 0..len {
                for kpos in 0..len {
                    ...
                    scores[qpos * len + kpos] =
                        acc + rel_bias[h * len * len + qpos * len + kpos];
                }
            }
...
mod tests {
    ...
    fn rel_bias_embeds_sign_by_half_bucket_offset() {
        ...
        let rb = Tensor { data, rows: buckets, cols: heads };
        ...
        for h in 0..heads {
            for q in 0..4 {
                for k in 0..4 {
                    ...
                    assert!((got - expect).abs() < 1e-6, "h={h} q={q} k={k}: {got} vs {expect}");
                }
            }
        }
    }
...
mod stream_tests {
    ...
    fn write_t5(dir: &Path, cfg: &T5Config, gated: bool) {
        ...
        for i in 0..cfg.num_layers {
            ...
            for p in ["q", "k", "v", "o"] {
                named.push((
                    format!("{b}.layer.0.SelfAttention.{p}.weight"),
                    vec![d, d],
                ));
            }
    ...
    fn stage_f16_is_bit_identical_to_decode_then_rne() {
        ...
        assert!(plan.gated, "wi_0/wi_1 naming must be detected as v1.1 gated");
        ...
        for (name, rows, cols) in &linears {
            let want: Vec<u16> = plan
                ...
                .collect();
            let got = plan.stage_f16(&src, name, *rows, *cols, &mut stage).unwrap();
    ...
    fn a_missing_tensor_is_named_in_the_error() {
        ...
        let mut stage = F16Stage::new();
        let err = plan
            .stage_f16(&src, "encoder.block.9.layer.0.SelfAttention.q.weight", 8, 8, &mut stage)
            .unwrap_err();

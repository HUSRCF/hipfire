...
fn run_modulate(gpu: &mut Gpu, n_rows: usize, d: usize, label: &str) -> usize {
    let x: Vec<f32> = (0..n_rows * d).map(|i| (((i * 7919) % 509) as f32 - 254.0) * 0.01).collect();
    let shift: Vec<f32> = (0..d).map(|i| (((i * 2027) % 97) as f32 - 48.0) * 0.01).collect();
    let scale: Vec<f32> = (0..d).map(|i| (((i * 7013) % 103) as f32 - 51.0) * 0.02).collect();
    ...
    gpu.modulate_f32(&g_x, &g_sh, &g_sc, &g_out, n_rows, d).unwrap();
    let got = gpu.download_f32(&g_out).unwrap();

    let max_want = want.iter().fold(0.0f32, |m, &x| m.max(x.abs())).max(1e-12);
    let max_err = got.iter().zip(want.iter()).fold(0.0f32, |m, (a, b)| m.max((a - b).abs()));
...
fn run_gated_add(gpu: &mut Gpu, n_rows: usize, d: usize, label: &str) -> usize {
    let acc0: Vec<f32> = (0..n_rows * d).map(|i| (((i * 1009) % 211) as f32 - 105.0) * 0.01).collect();
    let gate: Vec<f32> = (0..d).map(|i| (((i * 7013) % 103) as f32 - 51.0) * 0.02).collect();
    let x: Vec<f32> = (0..n_rows * d).map(|i| (((i * 4001) % 89) as f32 - 44.0) * 0.03).collect();
    let want: Vec<f32> = (0..n_rows * d)
...
fn main() {
    ...
    // In-place (out aliases x) modulation soundness, small shape.
    {
        let d = 96usize;
        let n_rows = 16usize;
        let x: Vec<f32> = (0..n_rows * d).map(|i| (((i * 1583) % 127) as f32 - 63.0) * 0.03).collect();
        let shift: Vec<f32> = (0..d).map(|i| (((i * 2027) % 97) as f32 - 48.0) * 0.02).collect();
        let scale: Vec<f32> = (0..d).map(|i| (((i * 7013) % 103) as f32 - 51.0) * 0.01).collect();
        let want: Vec<f32> = (0..n_rows * d)
            .map(|idx| x[idx] * (1.0 + scale[idx % d]) + shift[idx % d])
            .collect();
        let g_x = gpu.upload_f32(&x, &[n_rows, d]).unwrap();
        let g_sh = gpu.upload_f32(&shift, &[d]).unwrap();
        let g_sc = gpu.upload_f32(&scale, &[d]).unwrap();
        gpu.modulate_f32(&g_x, &g_sh, &g_sc, &g_x, n_rows, d).unwrap(); // in-place

...
fn main() {
    ...
    if let Ok(spec) = std::env::var("AB") {
        ...
        let [a, b] = names.as_slice() else {
            panic!("AB expects exactly two comma-separated variant names, e.g. AB=v2,v5; got {spec:?}");
        };
        let (a, b) = (*a, *b);
    ...
    let (t_vtk_of16, t_vtk_qf16, t_vtk_both) = if std::env::var("SKIP_VTK").is_ok() || !include_superseded {

...
fn conv_route_banded(
    ...
    let hw = h * w;
    let k = c_in * 9;
    let y = gpu.alloc_tensor(&[c_out * hw], DType::F32).expect("alloc y");
    let cols = gpu
...
const PARITY_SHAPES: &[Shape] = &[
    ...
    Shape { c_in: 16, c_out: 512, h: 16, w: 16 },
    // The deepest real conv, K = 4608.
    Shape { c_in: 512, c_out: 512, h: 24, w: 24 },
    Shape { c_in: 256, c_out: 256, h: 32, w: 32 },
    Shape { c_in: 128, c_out: 128, h: 48, w: 64 },
    // Ragged spatial dims: partial LDS tiles on both axes, and a c_out that
    // is not the c_in.
    Shape { c_in: 128, c_out: 64, h: 37, w: 53 },
];
...
fn main() {
    ...
    for s in PARITY_SHAPES {
        ...
        println!(
            ...
            s.c_in, s.h, s.w, band_diff, if ok { "PASS" } else { "FAIL" }
        );
        failures += usize::from(!ok);
        ...
        if k % 64 == 0 {
            ...
            gpu.gemm_f16_x_f16_wmma(&w16, &lds_cols, &pos, s.c_out, k, hw)
                .expect("gemm 16");
            gpu.bias_add_f32(&pos, &bias, hw, s.c_out).expect("bias add");
        }
        let gemm_out = gpu
            .alloc_tensor(&[s.c_out * hw], DType::F32)
        ...
        println!(
            ...
            s.c_in, s.c_out, s.h, s.w, if ok { "PASS" } else { "FAIL" }
        );
        failures += usize::from(!ok);

        for (t, what) in [
            (x, "x"), (ref_cols, "ref"), (lds_cols, "lds"), (band_buf, "band"),
            (w32, "w32"), (w16, "w16"), (bias, "bias"), (direct, "direct"),
            (pos, "pos"), (gemm_out, "gemm_out"),
        ] {
            gpu.free_tensor(t).unwrap_or_else(|e| panic!("free {what}: {e:?}"));
        }
    }
    ...
    for (m, n, band) in [(37 * 53usize, 128usize, 8 * 53usize), (20 * 64, 256, 6 * 64)] {
        let src = gpu
            .upload_f32(&fill(m * n, 0x7ab5), &[m, n])
            .expect("upload src");
        let whole_t = gpu.alloc_tensor(&[n * m], DType::F32).expect("alloc whole");
        gpu.vae_transpose_f32(&src, &whole_t, m, n).expect("transpose whole");
        let want = gpu.download_f32(&whole_t).expect("download whole");

        let banded_t = gpu.alloc_tensor(&[n * m], DType::F32).expect("alloc banded");
        let mut off = 0usize;
        while off < m {
    ...
    for (c, hw, groups) in [(512usize, 128 * 128usize, 32usize), (128, 64 * 64, 32)] {
        ...
        let fused = gpu.alloc_tensor(&[c * hw], DType::F32).expect("alloc fused");
...
fn bench_im2col(gpu: &mut Gpu) {
    ...
    let shapes = [(512usize, 128usize, 128usize), (256, 512, 512), (128, 1024, 1024)];
    let iters = 5;

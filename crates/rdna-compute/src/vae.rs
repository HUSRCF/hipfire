...
fn im2col_tile(c_in: usize) -> HipResult<(usize, usize, usize)> {
    if let Ok(spec) = std::env::var("HIPFIRE_VAE_IM2COL_TILE") {
...
impl Gpu {
    ...
    pub fn vae_im2col_f16_band(
        ...
        let map = std::env::var("HIPFIRE_VAE_IM2COL_MAP").unwrap_or_default();
    ...
    pub fn vae_transpose_f32_banded(
        ...
        self.bind_thread()?;
        let tiled = std::env::var("HIPFIRE_VAE_TRANSPOSE").is_ok_and(|v| v == "tiled");
        let entry = if tiled {

...
async fn handle_request(
    ...
    match (method, path.as_str()) {
        ...
        (Method::POST, "/v1/images/edits") => {
            ...
            let body_val = match parse_multipart(&bytes, &boundary).and_then(|(f, i)| edits_form_to_body(f, i)) {
                Ok(v) => v,
...
async fn handle_images_generations(
    ...
    if n != 1 {
        return Err(format!(
            "n={n} unsupported: one image per request"
        ));
    }
...
fn parse_multipart(body: &[u8], boundary: &str) -> Result<(Vec<(String, String)>, Vec<Vec<u8>>), String> {
...
mod tests {
    ...
    fn multipart_edit_form_carries_image_bytes_not_paths() {
        ...
        assert_eq!(fields, vec![("prompt".to_string(), "make it blue".to_string()), ("steps".to_string(), "4".to_string())]);
        ...
        assert!(parse_multipart(&other, boundary).unwrap_err().contains("only `image`"));
        assert!(edits_form_to_body(vec![], vec![]).unwrap_err().contains("required"));
    }
}

...
use hipfire_quantize::float16::{bf16_to_f32, f32_to_f16, f16_to_f32};
use rayon::prelude::*;
...
fn pack_component(
    ...
    let src = SafetensorsSource::open(&dir)
        .map_err(|e| format!("flux pack: open {sub}/: {e:?}"))?;
    ...
    if component == FluxComponent::Transformer {
        ...
        if parsed_is_flux2 != expected_is_flux2 {
            return Err(format!(
                ...
                if parsed_is_flux2 { "FLUX.2 (klein)" } else { "FLUX.1" },
                if expected_is_flux2 { "FLUX.2 (klein)" } else { "FLUX.1" },
                if parsed_is_flux2 { ARCH_FLUX2_TRUNK } else { ARCH_FLUX1_TRUNK },
                if expected_is_flux2 { ARCH_FLUX2_TRUNK } else { ARCH_FLUX1_TRUNK },
            ));
        }
    }
...
fn to_f32_bytes(info: &hipfire_runtime::model_source::TensorInfo, bytes: &[u8]) -> Result<Vec<u8>, String> {
...
fn to_f16_bytes(info: &hipfire_runtime::model_source::TensorInfo, bytes: &[u8]) -> Result<Vec<u8>, String> {
    let info_name = &info.name;
    match info.dtype.as_str() {
        "F16" => Ok(bytes.to_vec()),
        "BF16" => Ok(bytes
...
fn component_metadata(
    ...
    m.insert(
        "family".into(),
        serde_json::json!(if family == PipeFamily::Flux2 { "flux2" } else { "flux1" }),
    );
    m.insert(
        "config".into(),
    ...
    if component == FluxComponent::Transformer {
        ...
        let tok = if family == PipeFamily::Flux1 {
            let t5_tok =
                std::fs::read_to_string(pipe.join("tokenizer_2/tokenizer.json"))
                    .map_err(|e| format!("flux pack: tokenizer_2/tokenizer.json: {e}"))?;
            let clip_vocab =
                std::fs::read_to_string(pipe.join("tokenizer/vocab.json"))
                    .map_err(|e| format!("flux pack: tokenizer/vocab.json: {e}"))?;
            let clip_merges =
                std::fs::read_to_string(pipe.join("tokenizer/merges.txt"))
...
mod tests {
    ...
    fn flux_pack_round_trip() {
        ...
        let (wi, wb) = trunk.tensor_data("double_blocks.0.img_attn.qkv.weight").unwrap();
        assert_eq!(wi.quant_type, 1, "weight packs as F16");
        assert_eq!(wb.len(), 384 * 128 * 2);
        let (bi, bb) = trunk.tensor_data("single_blocks.0.modulation.lin.bias").unwrap();
        ...
        assert!(meta["tokenizer"]["clip_merges"].as_str().unwrap().starts_with("#version"));

...
pub(crate) fn run() {
    ...
    let input_dir = args.input.as_deref().expect("--input is required unless --flux-pipe is given");
...
fn run_qwen3_dspark(args: &QuantizeArgs) {
    let input_dir = Path::new(args.input.as_deref().expect("--input is required unless --flux-pipe is given"));
